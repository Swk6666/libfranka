...
def main():
    parser = argparse.ArgumentParser(description="Plot Cartesian acceleration trajectory.")
    ...
    for i, (axis_name, axis_label) in enumerate(zip(axis_names, axis_labels)):
        ...
        if desired_pos[axis_name] is not None:
            ...
            desired_acc = compute_derivative(desired_vel, time)

        if direct_acc[axis_name] is not None:
            actual_acc = direct_acc[axis_name]
        elif actual_pos[axis_name] is not None:
            actual_vel = compute_derivative(actual_pos[axis_name], time)
            actual_acc = compute_derivative(actual_vel, time)
        else:
            raise KeyError(f"Missing data for {axis_name}-axis acceleration")

        if desired_acc is not None:
            ax.plot(time, desired_acc, color=colors["desired"], linewidth=1.5,
                    alpha=0.9, label="Desired")
        ax.plot(time, actual_acc, color=colors["actual"], linewidth=1.5,
                alpha=0.9, label="Actual")

        ax.set_ylabel(f"{axis_label} (m/s^2)", fontsize=11)
        ax.grid(True, alpha=0.3)
        ax.legend(loc="upper right")

        max_val = np.max(actual_acc)
        min_val = np.min(actual_acc)
        ax.text(0.02, 0.95, f"Max: {max_val:.4f} m/s^2\nMin: {min_val:.4f} m/s^2",
